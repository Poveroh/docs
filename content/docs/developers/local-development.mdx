...
If you have `MacOS`, `Linux`, or `Windows`, you can follow these instructions to set up your local development environment. All scripts are written in JavaScript and can be run using `npm` or `yarn`, so it runs on any platform that supports Node.js. 
